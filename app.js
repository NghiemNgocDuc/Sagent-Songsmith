...
const API_BASE = window.SAGENT_CONFIG?.apiBase || "http://127.0.0.1:8000";
...
fetchSongUrlBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    const message = error instanceof TypeError
      ? `Could not reach the backend at ${API_BASE}. Start the backend server and try again.`
      : error.message;
    alert(message);
    resetStatus();
    songUrlNote.textContent = error instanceof TypeError
      ? `The browser could not reach the backend at ${API_BASE}. Start the backend server, then try the link again.`
